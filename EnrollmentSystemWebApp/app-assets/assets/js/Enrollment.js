...
$(function () {
    ...
    $('#cbJustCausa').change(function () {
        if (parseInt($(this).val()) === 7) {
            $('#JustCauseCommentSection').attr('hidden', false);
        } else {
            $('#JustCauseCommentSection').attr('hidden', true);
        }
    ...
    $('body').on('click', '#btnSearch', function (e) {
        ...
        if ($("#cbJustCausa").val() === NotApllyJustCauseId && $("#txtCommentJustCausa").val() === "") {
            swal({
                title: i18next.t('common_warning'),
                text: i18next.t('Enrollment_InsertReasonJustCause'),
                type: "warning",
                confirmButtonText: "OK"
            });
            return;
    ...
    $('body').on('click', '#btnClean', function (e) {
        $('#txtPhName').val('');
        $('#txtNPI').val('');
        $("#cbSpeciality").val('').trigger("change");
        $("#cbPmg").val('').trigger("change");
        $("#cbMco").val('').trigger("change");
        $("#cbMunicipality").val('').trigger("change");
    });

    function cancelJustCause() {
        if ($('#justCauseCheck').is(":checked")) {
            ChangeEnrollmentEnabledJustCause();
        } else {
            $('#divReasonJustCause').hide();
            $('#txtCommentJustCausa').val("");
            $('#cbJustCausa').val('');
    ...
    function BuildRequestPcp() {
        ...
        NPI = $("#txtNPI").val();
        JustCauseComment = $('#txtCommentJustCausa').val();
        ...
        entityRequest.MunicipalityId = $("#cbMunicipality").val();
        entityRequest.ShowForChangeEnrollmentProcess = true;
        entityRequest.JustCauseComment = JustCauseComment;
    ...
    function BuildRequestPcpApply() {
        ...
        request.JustCauseComment = $('#txtCommentJustCausa').val();
        return request;
    }
...
function ChangeEnrollmentEnabledJustCause() {
    $.ajax({
        type: 'GET',
        url: urlChangeEnrollmentEnabledJustCause,
        contentType: 'application/json; charset=utf-8',
        dataType: "json",
        data: {
            MemberId: ApplicationMemberID
        ...
        success: function (data) {
            if (data.code !== 0) {
                ...
                swal({
                    title: i18next.t('common_warning'),
                    text: i18next.t('Enrollment_NotEnabledJustCause'),
                    type: "warning",
                    confirmButtonText: "OK"
                });
            } else {
        ...
        success: function (data) {
            ...
            if (data.code == 0) {
                if (data.recordsTotal > 0) {
                    ...
                    ActualPcp = obj?.PCP?.FullName;
                    ActualPmg = obj?.PMG?.PmgName;
                    ActualNPI = obj.PCPId;
                    ActualPmgId = obj.PMGId;
                    $("#txtContactMPI").val(obj.MemberFullName);
                    $("#txtBeneficiary").val(obj.MemberFullName);
                    PersonFullname = obj.MemberFullName;
                    ...
                    $("#txtEfectivityDate").val(formatDateEN(obj.MCOEffectiveDate));
                    $("#txtCertificationDate").val(formatDateEN(obj.CertificationDate));
                    $("#txtComment").val(obj.Reason);
                    $("#txtContactMPITab2").val(obj.MemberFullName);
                    $("#txtContactMPITab3").val(obj.MemberFullName);
                    $("#Last4SSN").val(obj.Last4SSN);
                    $("#txtDateOfBirth").val(formatDateEN(obj.DateOfBirth));
                    $("#txtContactFullName").val(obj.Family.ContactFullName);
...
function LoadTableSearchHistory(tableSection, JSonData) {
    ...
    $(tableSection).DataTable({
        ...
        "columns": [
            ...
            { "title": '<span data-i18n="Enrollment_ChangeSource">' + i18next.t('Enrollment_ChangeSource') + '</span>', "data": "MCOModifiedSource", "name": "MCOModifiedSource" },
            {
