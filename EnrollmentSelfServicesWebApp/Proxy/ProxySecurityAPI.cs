...
namespace EnrrolmentSelfServicesWebApp.Proxy
{
    public class ProxySecurityAPI
    {
        ...
        public async Task<EResponseBase<User_Response_v1>> ChangePasswordExternal(CustomConfigurationLib config,  User_Request_Filters_v12 request)
        {
            using (CustomProxyREST<User_Request_Filters_v12, User_Response_v1> service = new CustomProxyREST<User_Request_Filters_v12, User_Response_v1>(config))
            {
                string transaction = string.Empty;
                string dataRequest = JsonConvert.SerializeObject(null);
                EResponseBase<User_Response_v1> response = await service.Post(config.SecurityAPI_URLBase,
                                                 config.SecurityAPI_ServicePrefix,
                                                 config.SecurityAPI_IdentityController,
                                                 config.SecurityAPI_Identity_ChangePasswordActionExternal,
                                                 null,
                                                 null,
                                                 transaction.GetTransaction().Id,
                                                 AppConstants.Web,
        ...
        public async Task<EResponseBase<SimpleEntity>> SendUserRegister(CustomConfigurationLib config, EmailRequestV1 request)
        {
            using (CustomProxyREST<EmailRequestV1, SimpleEntity> service = new CustomProxyREST<EmailRequestV1, SimpleEntity>(config))
            {
                string transaction = string.Empty;
                ...
                EResponseBase<SimpleEntity> response = await service.Post(config.SecurityAPI_URLBase,
                                                 config.SecurityAPI_ServicePrefix,
                                                 config.SecurityAPI_MailController,
                                                 config.SecurityAPI_Mail_SendUserRegister,
                                                 null,
                                                 null,
                                                 transaction.GetTransaction().Id,
                                                 AppConstants.Web,
        ...
        public async Task<EResponseBase<User_Response_v1>> GetId(Transaction transaction, ICustomLog logger, CustomConfigurationLib config, string token, int UserId)
        {
            using (log4net.NDC.Push(RequestHelpers.AuditUserData()))
            {
                using (CustomProxyREST<User_Response_v1> service = new CustomProxyREST<User_Response_v1>(config))
                {
                    ...
                    logger.Info(string.Format("URL: {0} {1}",
                                           ProxyBase.GetURL(config.SecurityAPI_URLBase,
                                                 config.SecurityAPI_ServicePrefix,
                                                 config.SecurityAPI_UserController,
                                                 config.SecurityAPI_User_GetById
                                                            ),
                                           "(GET)"));
                    logger.Print_Request(null);
                    EResponseBase<User_Response_v1> response = await service.Get(config.SecurityAPI_URLBase,
                                                    config.SecurityAPI_ServicePrefix,
                                                    config.SecurityAPI_UserController,
                                                    config.SecurityAPI_User_GetById,
                                                    null,
        ...
        public async Task<EResponseBase<User_Response_v1>> SetUser(Transaction transaction, ICustomLog logger, CustomConfigurationLib config, string token, User_Request_v1 request)
        {
            using (log4net.NDC.Push(RequestHelpers.AuditUserData()))
            {
                using (CustomProxyREST<User_Request_v1, User_Response_v1> service = new CustomProxyREST<User_Request_v1, User_Response_v1>(config))
                {
                    logger.Print_InitMethod();
                    logger.Info(string.Format("URL: {0} {1}",
                                           ProxyBase.GetURL(config.SecurityAPI_URLBase,
                                                    config.SecurityAPI_ServicePrefix,
                                                    config.SecurityAPI_UserController,
                                                    config.SecurityAPI_User_SetUser
                                                            ),
                                           "(GET)"));
                    logger.Print_Request(null);
                    EResponseBase<User_Response_v1> response = await service.Post(config.SecurityAPI_URLBase,
